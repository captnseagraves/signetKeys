...
contract VerifyingPaymaster is BasePaymaster, Ownable2Step {
    ...
    error SenderTokenBalanceTooLow(
        address token,
        uint256 balance,
    ...
    function transferOwnership(
        address newOwner
    ) public override(Ownable2Step, Ownable) onlyOwner {
    ...
    function _validatePaymasterUserOp(
        ...
        if (paymasterData.token != address(0)) {
            if (
                ...
                uint256 maxTokenCost = _calculateTokenCost(
                    maxCost + paymasterData.postOpGas * userOp.maxFeePerGas,
                ...
                if (paymasterData.precheckBalance) {
                    uint256 balance = ERC20(paymasterData.token).balanceOf(
                        userOp.sender
                    );
                    if (balance < maxTokenCost) {
                        revert SenderTokenBalanceTooLow(
                            paymasterData.token,
                            balance,
                            maxTokenCost
                        );
                ...
                if (paymasterData.prepaymentRequired) {
                    ...
                    ERC20(paymasterData.token).safeTransferFrom(
                        userOp.sender,
                        address(this),
                        maxTokenCost
                    );
    ...
    function _postOp(
        ...
        if (c.token != address(0) && mode != PostOpMode.postOpReverted) {
            ...
            uint256 gasPrice = _min(
                c.maxFeePerGas,
                c.maxPriorityFeePerGas + block.basefee
            );
            uint256 actualTokenCost = _calculateTokenCost(
                actualGasCost + c.postOpGas * gasPrice,
                c.exchangeRate
            );
            ...
            if (c.prepaidAmount == 0) {
                ERC20(c.token).safeTransferFrom(
                    c.sender,
                    c.receiver,
                    actualTokenCost
                );
            ...
            emit UserOperationSponsoredWithERC20(
                c.userOpHash,
                c.sponsorUUID,
                c.token,
                c.receiver,
    ...
    function _transferOwnership(
        address newOwner
    ) internal override(Ownable2Step, Ownable) {
