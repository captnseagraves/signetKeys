...
contract TestIsValidSignature is SmartWalletTestBase {
    ...
    function testSmartWalletSigner() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(
            signerPrivateKey,
            otherAccount.replaySafeHash(toSign)
        );
        bytes memory signature = abi.encodePacked(r, s, v);

        CoinbaseSmartWallet.SignatureWrapper
            memory wrapperForOtherAccount = CoinbaseSmartWallet
    ...
    function testValidateSignatureWithPasskeySignerFailsBadOwnerIndex() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MultiOwnable.InvalidOwnerBytesLength.selector,
                hex""
            )
        );
    ...
    function testValidateSignatureWithPasskeySignerFailsWithWrongBadSignature()
        public
    {
        bytes32 hash = 0x15fa6f8c855db1dccbb8a42eef3a7b83f11d29758e84aed37312527165d5eec5;
        bytes32 challenge = account.replaySafeHash(hash);
    ...
    function testValidateSignatureWithEOASigner() public {
        ...
        bytes memory signature = abi.encodePacked(r, s, v);
        bytes4 ret = account.isValidSignature(
            hash,
            abi.encode(CoinbaseSmartWallet.SignatureWrapper(0, signature))
        );
        assertEq(ret, bytes4(0x1626ba7e));
    }
    ...
    function testRevertsIfEthereumSignatureButWrongOwnerLength() public {
        bytes32 hash = 0x15fa6f8c855db1dccbb8a42eef3a7b83f11d29758e84aed37312527165d5eec5;
        bytes32 toSign = SignatureCheckerLib.toEthSignedMessageHash(
            account.replaySafeHash(hash)
        );
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(signerPrivateKey, toSign);
        bytes memory signature = abi.encodePacked(r, s, v);
        vm.expectRevert();
        account.isValidSignature(
            hash,
            abi.encode(CoinbaseSmartWallet.SignatureWrapper(1, signature))
        );
    }
    ...
    function testRevertsIfOwnerIsInvalidEthereumAddress() public {
        ...
        bytes32 slot_ownerAtIndex = bytes32(
            uint256(
                0x97e2c6aad4ce5d562ebfaa00db6b9e0fb66ea5d8162ed5b243f51a2e03086f00
            ) + 2
        ); // MUTLI_OWNABLE_STORAGE_LOCATION
        // + 2
        bytes32 slot_ownerAtIndex_zeroIndex = bytes32(
            uint256(
                keccak256(
                    abi.encodePacked(
                        keccak256(abi.encode(0, slot_ownerAtIndex))
                    )
                )
            )
        );
