...
contract TestExecuteWithoutChainIdValidation is SmartWalletTestBase, TestPlus {
    function testExecuteBatch() public {
        ...
        CoinbaseSmartWallet.Call[]
            memory calls = new CoinbaseSmartWallet.Call[](2);
        ...
        assertEq(
            MockTarget(calls[0].target).datahash(),
            keccak256(_randomBytes(111))
        );
        assertEq(
            MockTarget(calls[1].target).datahash(),
            keccak256(_randomBytes(222))
        );
        assertEq(calls[0].target.balance, 123);
        assertEq(calls[1].target.balance, 456);

        calls[1].data = abi.encodeWithSignature(
            "revertWithTargetError(bytes)",
            _randomBytes(111)
        );
        vm.expectRevert(
            abi.encodeWithSignature("TargetError(bytes)", _randomBytes(111))
        );
        account.executeBatch(calls);
    }
    ...
    function testExecuteBatch(uint256 r) public {
        ...
        unchecked {
            uint256 n = r & 3;
            CoinbaseSmartWallet.Call[]
                memory calls = new CoinbaseSmartWallet.Call[](n);
            ...
            if (_random() & 1 == 0) {
                MockCoinbaseSmartWallet(payable(address(account))).executeBatch(
                    _random(),
                    calls
                );
            } else {
            ...
            for (uint256 i; i != n; ++i) {
                ...
                assertEq(
                    MockTarget(calls[i].target).datahash(),
                    keccak256(_randomBytes(v))
                );
